...
- **Проверено:** NuGet ругается NU1903 про ImageSharp 2.1.8 с пометкой
  `[TargetFramework=net10.0-windows]` - это **переигрывание restore-варнинга** на
  каждую ногу, а не протечка. `project.assets.json` показывает честно:
  net48 -> 2.1.8, net10 -> 3.1.12.

## СЛЕДУЮЩИЙ ШАГ

Этап 9 (MKV/ISO - **сперва закрыть O-ISO-1..7 с владельцем**), этап 10 (урезка x86
по `FEATURE_FULL` - **имя и разрядность уже сделаны**; заодно выкинет из x86
уязвимый ImageSharp 2.1.8), этап 11-остаток (winget/Store при релизе).
