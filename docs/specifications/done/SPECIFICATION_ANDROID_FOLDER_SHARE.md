...
4. **No *silent* elevation** at runtime. No Windows service anywhere. Autostart stays a runtime opt-in (an install-time HKCU write can land in the wrong hive when the install is elevated/machine-scoped, as winget validation is). **Scoped exception (owner-approved, 2026-07, [SPECIFICATION_SHARE_SERVER_OPTIN_INSTALL.md](SPECIFICATION_SHARE_SERVER_OPTIN_INSTALL.md)):** the one Windows Firewall exception that actually makes a share reachable IS now added - but only as a deliberate opt-in, never silently: (a) by the elevated installer when the new "Install server features" checkbox is ticked (recorded as a hive-safe machine marker file, not an HKCU write), or (b) at runtime behind a single explicit UAC prompt (the `enable-share-server.ps1` helper via `ShellExecute("runas")`), which then records the per-user HKCU flag. The whole Share UI + the worker spawn are gated behind `ServerFeatures.IsEnabled()` (marker OR HKCU flag OR packaged); until the user consents the app is a pure viewer/sorter. Still no Windows service, still no *silent* elevation, still no install-time HKCU consent write.
...
**Multi-path export (S1006/S1013/S1014, 2026-07) - additive, still `schemaVersion` 1.** These are backward-compatible extensions (old parsers ignore the new bits), so they do NOT bump the version and the canonical vector above is unchanged; full detail in `CONFIG_FORMAT.md` and [SPECIFICATION_QR_IMPORT_ANDROID.md](SPECIFICATION_QR_IMPORT_ANDROID.md):
- **`accessPaths` order is now `lan` -> `ipv6` -> `portforward`**, with a new `"ipv6"` kind (globally-routable IPv6 on the same listen port, bypasses NAT/CGNAT; bare literal, no brackets). Any entry that is not actually usable is omitted. The Android side **races all entries** (S1006) and uses whichever connects, preferring LAN - it must not trust `accessPaths[0]` alone. `ShareConfigBuilder` fills `lan` from `WorkerReachability.LanAddress` (else the hardened `NetworkInfo.LocalIPv4()` fallback, so a real LAN adapter is never dropped), `ipv6` from `reach.Ipv6Address`, `portforward` from the external host.
- **Optional top-level `accessNote`** (string): a short localized reachability line the phone shows on connection failure (S1014). Built LITE-side by `ShareText.AccessNote` from the reachability state (honest external connect-back result / CGNAT / IPv6-only / LAN-only); the worker's reference exporter omits it to keep the canonical vector byte-stable.
- **mDNS TXT for LAN rediscovery (S1013):** the worker announces `_sftp-fms._tcp` with TXT `fp=<SHA256:...>` (== `hostKeyFingerprintSha256`), `name=`, `v=1`, on the LAN listen port, for the whole share lifetime. The phone matches a discovered service to a pinned resource by `fp` and reconnects on the live LAN address even when the scanned QR's address went stale.
- **Honest reachability:** the worker verifies a mapped port-forward with an external connect-back (check-host.net) and reports `ipv6Address` / `externalPortChecked` / `externalPortOpen` in the IPC `reachability` (see `internal/netaccess`).
