...
>
> **Where the console lives, 2026-08-15:** `Share_Hosting_Form.vb` no longer exists as a window. The console is the **"Хостинг" collapsible group of `Share_Settings_Form`** ([Share_Settings_Form.Hosting.vb](../../../src/FastMediaSorterCompanion/Forms/Share_Settings_Form.Hosting.vb)), its body moved verbatim. The compact-window work ([SPECIFICATION_SHARE_MANAGER_COMPACT_WINDOW.md](SPECIFICATION_SHARE_MANAGER_COMPACT_WINDOW.md) §12.6) had left the settings window as its only caller, and the extra window put an elevated action four levels deep. **Nothing in this specification changes**: the same four states are kept apart on their own lines, the same actions exist, and every one of them still goes through the single auditable elevated helper behind a visible UAC prompt. Read "the Hosting console" below as that tab.
>
