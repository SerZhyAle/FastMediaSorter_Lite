...
Статус: **реализовано в основном** (Ф0-Ф5 + упаковка, 2026-07-16) - см. живой журнал
[PROGRESS_DOTNET10_MODERN_BUILD.md](../guides/PROGRESS_DOTNET10_MODERN_BUILD.md).
Дата: 2026-07-14, ревизия 3 (2026-07-16)

> **Ревизия 3 (2026-07-16) - что реально сделано и чем это отличается от плана.**
> Ветка `feature/dotnet10-modern`. **Сделано:** Ф0 (швы `#If NETFRAMEWORK` +
> `src/Imaging/` IImageDecoder), Ф1 (`src/Modern/FastMediaSorter.Modern.vbproj`,
> те же исходники), Ф2 (ImageSharp 3.x - анимированный WEBP везде), Ф3 (видео
> только LibVLC, IE WebBrowser не инстанцируется), Ф4 (self-contained single-file
> publish), Ф5 (`build.ps1` + инсталлятор + CI + MSIX везут ОБА exe).
> **Отклонение от рев. 2 - форма поставки (директива владельца 2026-07-16):**
> legacy-вьювер раздаётся НЕ отдельно с сайта, а **вместе с мейнлайном, в одной
> папке**, под именем **`FastMediaSorter_x86.exe`** (`PlatformTarget=x86`);
> замороженное имя `FastMediaSorter_LITE.exe` перешло к modern-сборке (замена
> установленного exe на месте). Смежные библиотеки общие - каждый exe берёт своё по
> разрядности процесса. Инсталлятор ставит оба и целит ярлык/ассоциации в тот, что
> реально запустится на этой ОС.
> **НЕ сделано (осталось от рев. 2 и дополнений):** урезка x86-сборки по
> `#If FEATURE_FULL` (раздел 1.1) - x86-вьювер пока **полнофункциональный**, с OCR,
> переводом и Share; зум/панорама
> ([SPECIFICATION_ZOOM_PAN_CLASSIC_DOTNET10.md](SPECIFICATION_ZOOM_PAN_CLASSIC_DOTNET10.md));
> MKV/ISO ([SPECIFICATION_MKV_ISO_PLAYBACK_DOTNET10.md](done/SPECIFICATION_MKV_ISO_PLAYBACK_DOTNET10.md));
> перевод витрин winget/Store на новый ассет и релиз.
> Открытые вопросы O-1..O-8 закрыты - решения в журнале прогресса.
...
> **РЕВИЗИЯ 3 (2026-07-16) - раздел ниже устарел частично, читать вместе с этой
> врезкой.** Директива владельца «два exe рядом в одной папке + смежные библиотеки»
> отменила посылку, на которой стоял этот раздел: раз x86 лежит рядом с x64, то
> tessdata / libvlc / companion ставятся ради x64 **в любом случае**, и обещанная тут
> экономия «десятки МБ вместо сотен» испаряется - урезка сэкономила бы только сам
> exe (~3 МБ).
>
> **Решение владельца: резать из x86 ТОЛЬКО Share.** Причина не в весе, а в том, что
> Share там **физически не работает**: Companion - это `net10.0-windows` **x64**, он
> не стартует ни на Win7/8.1, ни на 32-битной Windows, то есть ровно там, ради чего
> x86 и существует. **OCR и перевод остаются** - в отличие от Share, они там
> работают, и отнимать их у единственного пользователя этого exe не за что.
>
> **O-7 (механизм отсечения) закрыт и оказался пустым вопросом.** Ни `FEATURE_FULL`,
> ни отдельной build-конфигурации не нужно: net48-сборка ровно одна и она И ЕСТЬ
> x86-фолбэк, так что `#If Not NETFRAMEWORK` уже называет отсекаемое точно. Вторая
> константа лишь добавила бы им способ разойтись.
>
> Сделано: `Main_Form.ShareLauncher.vb` и `Table_Form.ShareLauncher.vb` целиком под
> `#If Not NETFRAMEWORK` (проверено по бинарям: в x86 маркеров Share нет, в x64 есть);
> компонент `share` в инсталляторе получил `MinVersion: 10.0.14393` - **заодно чинит
> найденный баг**: `MinVersion=6.1` пускает Setup на Win7 и клал туда net10-x64-exe,
...
> пополнение к этой спецификации - [SPECIFICATION_MKV_ISO_PLAYBACK_DOTNET10.md](done/SPECIFICATION_MKV_ISO_PLAYBACK_DOTNET10.md).
