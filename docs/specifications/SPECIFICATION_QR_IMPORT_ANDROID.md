...
- Reject when `schemaVersion` **>** the highest you implement -> show "update the app" (do a partial import of nothing). `schemaVersion` **<** yours: only if you ever ship v2+ and keep back-compat; for now expect exactly `1`.
...
- [ ] Rejects `schemaVersion` > supported with an "update the app" message.
