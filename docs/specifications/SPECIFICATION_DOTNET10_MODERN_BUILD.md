...
- **Оставить текущую сборку как legacy** на .NET Framework 4.8 - заморозить,
  чинить только критичное, распространять с сайта для Windows 7/8.1 и машин без
  возможности поставить .NET 10.
...
- **Каналы витрин (winget, Microsoft Store) перевести на новую сборку**; сайт
  раздаёт обе (новая - основная, legacy - вторая ссылка «для старых Windows»).
...
- **Modern - мейнлайн**, держит замороженную базу имени ассета
  ...
  указывают winget и Store.
- **Legacy** получает отдельный суффикс, напр.
  `FastMediaSorter-<ver>-legacy-net48-windows-x64.zip`/`-setup.exe`, только на
  сайте/в GitHub-релизе, в витрины не идёт.
- Версия `YY.M.D.HHmm` общая для обеих (собираются из одного тега).

### 8.4 CI (`release.yml`)

1. Собрать legacy (как сейчас) и modern (`dotnet publish` self-contained).
2. Прогнать `Prepare-OcrOfflinePayload.ps1` для обеих (tessdata, обрезка arch).
3. Modern: Inno `setup.exe` + portable zip под замороженными именами.
4. Legacy: те же артефакты с суффиксом `-legacy-net48`.
5. Приложить все к GitHub-релизу; winget/Store-автоматика берёт modern-ассеты.

Инсталлятор `installer/FastMediaSorter.iss` параметризуется источником файлов
(`bin/Release` net48 vs `publish` net10) - один `.iss`, вход по `#define`.

### 8.5 Сайт

Страница загрузок: основная кнопка «Fast Media Sorter for Windows (Windows 10/11,
Server 2016+)» → modern; вторая ссылка «Для Windows 7/8.1 (legacy, .NET Framework
4.8)» → legacy. Обновить `index.html`/`docs/` (помнить про дубль корень+`docs/`).
