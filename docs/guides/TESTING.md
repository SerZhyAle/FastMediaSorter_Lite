...
exe» (.NET 10 мейнлайн + net48 x86-собрат): все три набора зелёные -
**12 + 36 тестов + 5 Go-пакетов, `Run-AllTests.ps1` exit 0**.
...
врезку вверху). Покрытая логика физически та же, что компилирует и net10-мейнлайн, -
но прогоняется она только под net48.
