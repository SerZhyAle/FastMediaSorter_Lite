...
| Набор | Результат |
|---|---|
| Вьювер, **net10** (мейнлайн `FastMediaSorter_LITE.exe`) | **18 тестов** |
| Вьювер, **net48** (`FastMediaSorter_x86.exe`) | **15 тестов** |
| Share Manager (net10) | 36 тестов |
| SFTP-воркер (Go) | 5 пакетов |

Разница 18 vs 15 - не пробел, а сам смысл швов: три теста в
`ImageDecoderSeamTests` живут под `#If Not NETFRAMEWORK`, потому что проверяют то,
чего net48 принципиально не умеет (см. ниже).
...
**Отдельно (не тесты, но всплыло при прогоне):** restore ругается `NU1903` -
у **ImageSharp 2.1.8** (его везёт net48-вьювер через `packages.config`) известны
уязвимости high-уровня. Мейнлайн на 3.1.12 их не касается. Это ещё один довод за
модерн-сборку и повод решить, что делать с x86-веткой: 3.x не таргетит net48, так
что варианты - жить с этим, урезать x86 по `FEATURE_FULL` (тогда ImageSharp там
вообще не нужен - см. эпик §1.1) или искать патч-версию 2.x.
