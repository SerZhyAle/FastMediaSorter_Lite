...
- **[src/Companion/](src/Companion)** - `WorkerIpc`/`WorkerProcess` (named-pipe JSON client + spawn/stop of the sidecar), `ShareController` (high-level async driver shared by the tab and the wizard), `ShareConfigBuilder` (builds the `.fmscfg` JSON + QR PNG on the LITE side), `ShareRootParams` (per-root schema-v2 export params + registry store), `AutostartManager` (channel-aware logon autostart), `NetworkInfo`/`RouterInfo`/`SftpProbe` (LAN/reachability/router-model detection), `MailSender` (Simple MAPI "email the .fmscfg" button), `ShareGuide`/`ShareText` (bundled port-forward guide + RU/EN copy)
- **[Table_Form.Share.vb](src/Table_Form.Share.vb)** - the Share settings tab (see Table_Form above)
- **[Share_Wizard_Form.vb](src/Share_Wizard_Form.vb)** / **[Share_Root_Params_Form.vb](src/Share_Root_Params_Form.vb)** / **[Qr_Zoom_Form.vb](src/Qr_Zoom_Form.vb)** - one-window "share this folder" quick wizard, the per-root resource-params dialog, and a click-to-enlarge QR window (the source PictureBox's image is cloned since async status polls keep rebuilding the original)
- **[Main_Form.ShareTray.vb](src/Main_Form.ShareTray.vb)** / **[Main_Form.ShareWizard.vb](src/Main_Form.ShareWizard.vb)** - tray icon with close-to-tray while serving, and the three wizard entry points (toolbar button, `Shift+S`, folder-box right-click)
- **[ShareSettings.vb](src/ShareSettings.vb)** - LITE-side UX prefs only (autostart opt-in, LAN-only export, exclude-password-from-export); the shared-folder list itself lives in the worker's own `shares.json`, not here
...
- **Distribution**: the worker ships as a sibling payload (`<app dir>\companion\fms-share-worker.exe`) in every channel - portable ZIP, Inno installer ([FastMediaSorter.iss](installer/FastMediaSorter.iss) `[Files]`, with [installer/stop-companion.ps1](installer/stop-companion.ps1) killing a running worker before Setup/Uninstall replaces files), and Store MSIX (`uap5:StartupTask` + `desktop2:windows.firewallRules` manifest extensions, since an HKCU Run write from inside an MSIX container is silently virtualized away). No Windows service anywhere (blocked by Store policy) and no runtime elevation.
