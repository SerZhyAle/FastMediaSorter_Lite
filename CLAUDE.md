...
See [SPECIFICATION_RENAME_FAST_MEDIA_SORTER_FOR_WINDOWS.md](docs/specifications/SPECIFICATION_RENAME_FAST_MEDIA_SORTER_FOR_WINDOWS.md).
...
- On `Startup`: a path-independent named mutex (`FastMediaSorterSingleInstanceMutex`) detects an already-running instance (VB's built-in `IsSingleInstance` only matches same exe path, so debug/release/renamed copies would each start their own). If found, the new process forwards its command-line file path to the running window via `WM_COPYDATA` and cancels (`e.Cancel = True`). A tray-resident instance has its window hidden, so `Process.MainWindowHandle` is zero - the sender falls back to `EnumWindows` over the process's top-level windows (only `Main_Form.WndProc` reacts to `WM_COPYDATA`, so sending to all of them is safe).
...
- **Bare relaunch** (no file argument) = the user re-ran the exe just to get the window back; both paths restore it instead of doing nothing: `StartupNextInstance` calls `Main_Form.RestoreMainWindow()` directly, the mutex path forwards `Main_Form.Show_Window_Command` (`::fms-show-window::`) which `External_message` intercepts. The window reappears with its current content (it was only hidden, never torn down) and - unlike a file open - DOES take focus (a bare launch has no other purpose). `RestoreMainWindow()` (in `Main_Form.ShareTray.vb`) leaves tray-resident mode, unminimizes and activates; the tray menu items reuse it. NB for testing: an external `WM_CLOSE` reads as `TaskManagerClosing` (real exit) - simulate the X button with `WM_SYSCOMMAND`+`SC_CLOSE` to get close-to-tray.
...
**[Table_Form.vb](src/Table_Form.vb)** - **Settings window** ("Настройки" / "Settings"), six tabs:
...
- **Поделиться / Share** (`Tab_Page_6`, built entirely in code in [Table_Form.Share.vb](src/Table_Form.Share.vb) - unlike `Tab_Page_5` it has no Designer-side TabPage at all) - the Android Folder Share feature (see below). A common left column (shared-folder list + Start/Stop + autostart) plus an inner two-page `TabControl` reframed by role (S1006): tab 1 = the ONE primary combined QR + `.fmscfg` (LAN + IPv6 + internet, the phone races them) with a "Только локальная сеть"/LAN-only toggle; tab 2 = internet-access setup guidance only (no duplicate code). Both built via `ShareConfigBuilder` (see "Multi-path export" below).
...
**Android Folder Share components** (see "Android Folder Share (Companion sidecar)" below):
- **[src/Companion/](src/Companion)** - `WorkerIpc`/`WorkerProcess` (named-pipe JSON client + spawn/stop of the sidecar), `ShareController` (high-level async driver shared by the tab and the wizard), `ShareConfigBuilder` (builds the `.fmscfg` JSON + QR PNG on the LITE side), `ShareRootParams` (per-root schema-v2 export params + registry store), `AutostartManager` (channel-aware logon autostart), `ServerFeatures` (the **opt-in gate** - see below), `NetworkInfo`/`RouterInfo`/`SftpProbe` (LAN/reachability/router-model detection), `MailSender` (Simple MAPI "email the .fmscfg" button), `ShareGuide`/`ShareText` (bundled port-forward guide + RU/EN copy)
- **[ServerFeatures.vb](src/Companion/ServerFeatures.vb)** - the opt-in enablement gate for the whole feature (see [SPECIFICATION_SHARE_SERVER_OPTIN_INSTALL.md](docs/specifications/SPECIFICATION_SHARE_SERVER_OPTIN_INSTALL.md)). Exposing folders over SFTP + punching a firewall hole is a deliberate admin-level action, so the entire Share surface (toolbar-button *action*, Settings tab *body*, tray, `Shift+S`, and the worker spawn itself) is gated behind `ServerFeatures.IsEnabled()` = **machine marker file** `<app dir>\companion\server-features.enabled` (written by the elevated installer) **OR** HKCU `Share_ServerFeaturesEnabled=1` (written by the deferred runtime opt-in) **OR** packaged (Store). Until enabled the app is a pure viewer/sorter and the worker is never started. `EnableViaElevation()` runs the single privileged step - a program-scoped inbound firewall allow for the worker exe - via one UAC prompt (`enable-share-server.ps1`, or a direct `netsh` fallback for a bare portable copy), then records the HKCU flag. The Share button/right-click/`Shift+S` stay visible when disabled but route to the enable dialog ([Share_Enable_Form.vb](src/Share_Enable_Form.vb)); the Settings Share tab shows an enablement panel instead of the sharing UI.
- **[Table_Form.Share.vb](src/Table_Form.Share.vb)** - the Share settings tab (see Table_Form above); shows the enablement panel vs. the full UI by the gate
- **[Share_Wizard_Form.vb](src/Share_Wizard_Form.vb)** / **[Share_Root_Params_Form.vb](src/Share_Root_Params_Form.vb)** / **[Qr_Zoom_Form.vb](src/Qr_Zoom_Form.vb)** / **[Share_Enable_Form.vb](src/Share_Enable_Form.vb)** - one-window "share this folder" quick wizard, the per-root resource-params dialog, a click-to-enlarge QR window (the source PictureBox's image is cloned since async status polls keep rebuilding the original), and the deferred opt-in confirm dialog
- **[Main_Form.ShareTray.vb](src/Main_Form.ShareTray.vb)** / **[Main_Form.ShareWizard.vb](src/Main_Form.ShareWizard.vb)** - tray icon with close-to-tray while serving, and the three gate-aware entry points (toolbar button, `Shift+S`, folder-box right-click) routed through `ActivateShareEntryPoint()`
- **[ShareSettings.vb](src/ShareSettings.vb)** - LITE-side UX prefs only (autostart opt-in, LAN-only export, exclude-password-from-export, plus a read-only mirror of the server-features flag owned by `ServerFeatures`); the shared-folder list itself lives in the worker's own `shares.json`, not here
...
- **QRCoder** (1.4.3) - renders the Android Folder Share QR codes (`ShareConfigBuilder`)
...
### Android Folder Share (Companion sidecar)
See [SPECIFICATION_ANDROID_FOLDER_SHARE.md](docs/specifications/SPECIFICATION_ANDROID_FOLDER_SHARE.md) for the full design (worker IPC protocol, `.fmscfg` schema, packaging plan) and [SPECIFICATION_QR_IMPORT_ANDROID.md](docs/specifications/SPECIFICATION_QR_IMPORT_ANDROID.md) for the frozen wire contract (written for the Android-side developer). Summary:
- **Architecture in one sentence**: LITE does not implement SFTP itself - it drives a bundled headless sidecar, `companion\fms-share-worker.exe` (prebuilt Go binary vendored at `payload/companion/`, **gitignored** - a fresh clone has no Share feature until it's placed there; built from a separate `fms_companion` repo, rebuild steps in the spec's Appendix C - **never build or hand-edit it from this repo**), over a named-pipe JSON control channel (`\\.\pipe\fms-companion`, schema-versioned request/response, one round-trip per connection). The worker owns the SFTP server, credentials, ed25519 host key, folder confinement, mDNS announce, and UPnP/NAT-PMP port mapping. `WorkerProcess.IsAvailable()` returns `False` when the payload is missing and the Share UI degrades gracefully (no crash, just "unavailable") - this is the normal state on a dev machine that hasn't fetched the payload.
...
- **Distribution**: the worker ships as a sibling payload (`<app dir>\companion\fms-share-worker.exe`) in every channel - portable ZIP, Inno installer ([FastMediaSorter.iss](installer/FastMediaSorter.iss) `[Files]`, with [installer/stop-companion.ps1](installer/stop-companion.ps1) killing a running worker before Setup/Uninstall replaces files), and Store MSIX (`uap5:StartupTask` + `desktop2:windows.firewallRules` manifest extensions, since an HKCU Run write from inside an MSIX container is silently virtualized away). No Windows service anywhere (blocked by Store policy).
- **Opt-in server features + firewall** ([ServerFeatures.vb](src/Companion/ServerFeatures.vb), [SPECIFICATION_SHARE_SERVER_OPTIN_INSTALL.md](docs/specifications/SPECIFICATION_SHARE_SERVER_OPTIN_INSTALL.md)): the bundled worker is dormant until the user opts in, because nothing in the LITE flow ever opened the Windows Firewall port (the worker's own `AddFirewallRule()` is dead code from the discontinued standalone GUI, never on the `cmd/worker` path) - so an exported public address timed out from outside. Enabling adds a program-scoped inbound firewall allow for the worker exe, either via the new installer checkbox (elevated install; records a hive-safe machine marker file) or the deferred in-app opt-in (one UAC prompt via `enable-share-server.ps1`; records the HKCU flag). This is the sole **owner-approved scoped exception** to Invariant #4's "no firewall/elevation" rule - never silent. The whole Share UI + the worker spawn are gated behind `ServerFeatures.IsEnabled()`.
- **Tray + close-to-tray lifecycle** ([Main_Form.ShareTray.vb](src/Main_Form.ShareTray.vb)): while a share is active, closing the main window hides it to the tray instead of exiting, so the worker - and its controls - stay reachable; the app only truly exits on an explicit Quit or once sharing is stopped.
...
2. **Run**: Open the built `.exe` and exercise features: load folders, navigate files, test slideshow, verify video playback. The Share tab/wizard need `payload/companion/fms-share-worker.exe` present (gitignored - see "Android Folder Share" below) to do more than show "unavailable"
