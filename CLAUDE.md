...
- Project identity: `RootNamespace = fmsl`, startup object `fmsl.My.MyApplication`. **The viewer ships as TWO exes side by side** (owner directive 2026-07-16, branch `feature/dotnet10-modern`): the .NET 10 x64 mainline **keeps the frozen `AssemblyName = FastMediaSorter_LITE`** (it replaces the installed exe in place), while the net48 project is now the lean x86 fallback for old/32-bit Windows with `AssemblyName = FastMediaSorter_x86` + `PlatformTarget = x86`. Both share one folder, one mutex, one registry hive and the adjacent libraries (codecs/OCR/worker) - each picks its native runtimes by process bitness out of the same tree (`libvlc\win-x64` vs `win-x86`, `x64\` vs `x86\`). See [PROGRESS_DOTNET10_MODERN_BUILD.md](docs/specifications/PROGRESS_DOTNET10_MODERN_BUILD.md).
