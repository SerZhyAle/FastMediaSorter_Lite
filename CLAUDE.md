...
4. Packages two **offline-ready** assets from that staged tree: a **portable ZIP** and an **Inno Setup installer EXE** (compiled via [installer/FastMediaSorter.iss](installer/FastMediaSorter.iss); Inno Setup installed on the runner with `choco install innosetup`). Each gets a `.sha256` sidecar. The installer ships **both** exes and points the Start-menu/desktop shortcut, the post-install launch and the file associations at whichever exe **runs on that OS** (`UseModernExe`: Windows 10 build >= 14393 gets the mainline, older gets `FastMediaSorter_x86.exe`); `MinVersion=6.1` still lets Setup run on Windows 7.
...
- [msix/AppxManifest.xml](msix/AppxManifest.xml) wraps the exe as a **full-trust** desktop app (`rescap:runFullTrust` keeps LibVLC/GDI+/file-access/local-HTTP working) and declares image **file associations** (the manifest equivalent of the Inno per-user registry writes). Identity (`Name`/`Publisher`/`PublisherDisplayName`) is `__PLACEHOLDER__` filled at pack time from Partner Center (Product ▸ Product identity) via `-IdentityName`/`-Publisher`/`-PublisherDisplayName` - must match **exactly** or upload is rejected.
