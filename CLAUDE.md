...
The viewer is **one app** shipped as **two exes in one folder**. Design: [SPECIFICATION_DOTNET10_MODERN_BUILD.md](docs/specifications/SPECIFICATION_DOTNET10_MODERN_BUILD.md); live implementation journal: [PROGRESS_DOTNET10_MODERN_BUILD.md](docs/specifications/PROGRESS_DOTNET10_MODERN_BUILD.md).
...
- **Both projects compile the SAME sources**: the modern vbproj links `..\**\*.vb`, so a file added under `src/` is picked up by both automatically. Divergences live behind `#If NETFRAMEWORK` seams, never in per-project file lists.
